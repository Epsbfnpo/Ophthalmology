...
def resnet50(pretrained=True, **kwargs):
    ...
    if pretrained and cfg.MODEL.PRETRAINED:
        ...
        print(f"========================================================")
        print(f"[ResNet] 正在从本地加载权重: {path}")
        if not os.path.exists(path):
            print(f"[FATAL ERROR] ❌ 本地权重文件不存在！")
            print(f"请下载 https://download.pytorch.org/models/resnet50-0676ba61.pth")
            print(f"并保存到: {path}")
            exit(1)
        try:
            state_dict = torch.load(path, map_location='cpu')
            msg = model.load_state_dict(state_dict, strict=False)
            print(f"[ResNet] ✅ 权重加载成功！")
            print(f"[ResNet] 未加载的层 (预期内): {msg.missing_keys}")
            print(f"========================================================")
        except Exception as e:
            print(f"[FATAL ERROR] ❌ 加载权重失败: {e}")
            exit(1)
    elif pretrained:
        print("[ResNet] 警告: 请求了预训练但未配置本地路径，尝试原有逻辑(可能会联网报错)...")
