...
class HFCFilter(nn.Module, abc.ABC):
    ...
    def forward(self, x, mask):
        ...
        for n in range(N):
            for c in range(C):  ### 实现Channel维度的归一化处理
                temp_res = (res * 256).int().float() / 256
                # print(temp_res)
                res_min, res_max = np.percentile(temp_res[n, c].detach().cpu().numpy(),  ###  np.percentile 计算一个多维数组的任意百分比分位数
                ...
                res[n, c] = (res[n, c] - res_min ) / max(res_max - res_min, 1e-12)  ### 防止分母、分子出现全为0的情况
                      
                # Deal with NaN values
                res[n, c][torch.isnan(res[n, c])] = 1

                # Clamp values to [0, 1]
                res[n, c] = torch.clip(res[n, c], 0., 1.)
                        # print(res[n, c] )
        if self.sub_mask:
            res = res * mask
        return res
...
class FourierButterworthHFCFilter(HFCFilter):
    ...
    def get_hfc(self, x, mask):
        ...
        # print(x.shape, x_fft.shape,self.filter_map.shape)  ## torch.Size([256, 256])
