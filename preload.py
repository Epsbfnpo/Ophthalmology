...
# 修改后的 generate_dataset 函数
def generate_dataset(cfg):
    # 1. 显式调用 auto_statistics 来计算均值方差
    if cfg.dataset.mean == 'auto' or cfg.dataset.std == 'auto':
        from data.builder import auto_statistics  # 确保引入这个函数
        ...
        cfg.dataset.std = std
        print(f"✅ Auto-calculated Mean: {mean}")
        print(f"✅ Auto-calculated Std:  {std}")

    # 2. 拿到数值后，再定义 transform
