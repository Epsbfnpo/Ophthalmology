...
class GDRNet(Algorithm):
    ...
    def __init__(self, num_classes, cfg):
        ...
        self.optimizer = torch.optim.Adam(
            [
                ...
                {"params": self.predictor.parameters(), 'fix_lr': False},  # Predictor 需要梯度
            ],
    ...
    def update(self, minibatch):
        ...
        with torch.no_grad():
            ...
            # 对 Strong Aug 进行 Mask
            image_new_patches = self.patchify(img_strong, block=bs)
            image_ori_patches = self.patchify(img_weak, block=bs)  # 这里其实通常不需要对 weak 做 mask，但 v0 逻辑中有互补 mask
        ...
        # FPT+ 必须返回 'features_list' (所有层的特征) 和 'features' (最终特征)
        ...
        # DINOv3-FD 特征 (用于正交损失)
        tra_feat = out_new['tra_feat']  # [B, N, D]
        # 这里的 tra_feat 需要处理一下，因为 FPT+ 返回的是 Sequence，我们需要一个向量来计算正交，或者对 Token 维度平均
        # DINOv3-FD 原文通常约束 TRA 和 TIA (Frozen Encoder 的输出) 正交
        # 简化版：我们假设 tra_feat 是我们学习到的 Adapter 特征，我们希望它包含任务信息
        ...
        # 4. Forward Pass (Teacher / EMA) - 这一步是 v0 关键
        with torch.no_grad():
            out_new_ema = self.network_ema(img_strong)
            features_new_ema_list = out_new_ema['features_list']  # 需要 nets.py 支持返回 list
            features_new_ema = out_new_ema['features']

        # 5. Projections
        # v0: z = Projector(feat) -> dim_in (2048)
        z1_sup = self.sample_target(self.projector(features_ori).detach(), label)
        z2_sup = self.sample_target(self.projector(features_new).detach(), label)
        ...
        # 归一化 (Loss 函数里可能已经做了，但 MASK_SIAM v0 在传入 Loss 前经常手动 norm)
        # 这里为了稳妥，按照 DahLoss 接口要求，一般传入 raw logits 或 features，Loss 内部处理。
        # 你的 DahLoss_Siam_Fastmoco_v0 似乎是在内部做 normalize，所以这里直接传 p/z 即可。

        # 准备传入 Loss 的列表
        # DahLoss_Siam_Fastmoco_v0 期待的 p1_orthmix 是一个 tensor list 或者切分后的
        ...
        # 7. Update Queue
        self.dequeue_and_enqueue(self.projector(features_new).detach(), label)  # Queue 存 Projector 的输出

        # 8. Main Loss Calculation
        loss, loss_dict = self.criterion(
            [output_new, output_ori, output_new_masked, output_ori_masked_c],
            [features_ori, features_new, features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix,
             z2_orthmix, p1_orthmix_list, p2_orthmix_list],
            [z_new_masked, z_ori_masked_c, p_new_masked, p_ori_masked_c],
            label, domain, random_matrix=None
        )

        # 9. Additional Loss 1: MASK_SIAM Feature Consistency (关键！)
        # 需要计算 Masked View 的中间层特征 和 EMA View 的中间层特征 的距离
        # 前提：nets.py 中的 FPTPlusNet 必须返回 'features_list'
        ...
        if features_new_masked_list is not None and features_new_ema_list is not None:
            # 假设 FPT+ 返回 4 层特征
            for i in range(len(features_new_masked_list)):
                # KD 温度参数，v0 默认为 1.0 或配置项
                loss_feat += self.E_dis(features_new_masked_list[i], features_new_ema_list[i].detach())

            # 加权
            loss += 1.0 * loss_feat  # 权重可调，v0 中通常直接加
            loss_dict['loss_feat'] = loss_feat.item()

        # 10. Additional Loss 2: DINOv3-FD Orthogonality Loss
        # 强制 tra_feat (任务相关) 与 dino_raw (原始特征) 正交
        # tra_feat: [B, N, D], dino_raw: [B, N, D]
        dino_raw = out_new['dino_raw'].detach()  # 原始特征不更新
        tra_feat = out_new['tra_feat']

        # 简单的正交损失：(|u . v| / (|u|*|v|))^2
        # 先 Flatten 或 Mean 池化
        u = tra_feat.mean(dim=1)
        v = dino_raw.mean(dim=1)
        cosine = F.cosine_similarity(u, v, dim=-1)
        loss_ortho = (cosine ** 2).mean()

        loss += 0.1 * loss_ortho  # DINOv3-FD 权重通常较小
