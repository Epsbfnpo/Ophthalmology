...
# --- [SPMix Archive] (Disabled) ---
# from .spmix_aug import SPMixAugmentation
# ----------------------------------

class GDRBench(Dataset):

    # [SPMix Archive] Added 'spmix_prob' arg but commented out to keep signature clean
    def __init__(self, root, source_domains, target_domains, mode, trans_basic=None, trans_mask=None, trans_fundus=None): # , spmix_prob=0.5):
        if root is None:
        ...
        # --- [SPMix Archive] Initialization Logic (Disabled) ---
    ...
    def __getitem__(self, index):
        try:
            ...
            # --- [SPMix Archive] Execution Logic (Disabled) ---
            # if getattr(self, 'use_spmix', False):
            #     try:
            #         # 1. Random Source Image
            #         import random
            ...
            #         data = mixed_data
            #
            #         # 3. Label Rewriting (Hard Label)
            #         if lam < 0.5:
            #             label = src_label
            #     except Exception as e:
            #         print(f"❌ [SPMix Error] Failed at index {index}: {e}")
            # --------------------------------------------------
