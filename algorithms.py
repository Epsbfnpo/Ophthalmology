...
#         return  -(target[(output_target_index == label)] * torch.log(x_m[(output_target_index == label)])).sum() / target.size()[0]
            ...
            # F.kl_div(x_m[(output_target_index == label)], target[(output_target_index == label)], reduction='batchmean') * (self.temperature ** 2)
    
    #  - 0.5  * (F.softmax(output_new / temperature, 1).detach() * F.log_softmax(output_new1 / temperature, 1)).sum() / output_new1.size()[0]
...
class LogitAdjust_KD_V2(nn.Module):
    ...
    def forward(self, x, target, label):
        ...
        return   -(target_t[(output_target_index == label)] * x_m[(output_target_index == label)]).sum() / target.size()[0] # F.kl_div(x_m[(output_target_index == label)], target_t[(output_target_index == label)], reduction='batchmean') * (self.temperature ** 2) # #
...
class GDRNet_MASK_SIAM(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        self.align1 = nn.Sequential(nn.Conv2d(self.network.n_outputs1, self.network.n_outputs, kernel_size=1, stride=1, padding=0),
                                        # nn.BatchNorm1d(feat_dim1),
                                        nn.ReLU(inplace=True), # hidden layer
                                        nn.Conv2d(self.network.n_outputs, self.network.n_outputs, kernel_size=1, stride=1, padding=0)
                                        )

        self.align2 = nn.Sequential(nn.Conv2d(self.network.n_outputs2, self.network.n_outputs, kernel_size=1, stride=1, padding=0),
                                        # nn.BatchNorm1d(feat_dim1),
                                        nn.ReLU(inplace=True), # hidden layer
                                        nn.Conv2d(self.network.n_outputs, self.network.n_outputs, kernel_size=1, stride=1, padding=0)
                                        )
        
        self.align3 = nn.Sequential(nn.Conv2d(self.network.n_outputs3, self.network.n_outputs, kernel_size=1, stride=1, padding=0),
                                        # nn.BatchNorm1d(feat_dim1),
                                        nn.ReLU(inplace=True), # hidden layer
                                        nn.Conv2d(self.network.n_outputs, self.network.n_outputs, kernel_size=1, stride=1, padding=0)
                                        )
        ...
        # init_weights_MLPHead(self.align1, init_method='He')
        # init_weights_MLPHead(self.align2, init_method='He')
        # init_weights_MLPHead(self.align3, init_method='He')
        ...
        self.optimizer = torch.optim.AdamW(
            ...
            # {"params":self.align1.parameters(), 'fix_lr': False},
            # {"params":self.align2.parameters(), 'fix_lr': False},
            # {"params":self.align3.parameters(), 'fix_lr': False},
    ...
    def update(self, minibatch):
        ...
        # image_new_feq, image_ori_feq = self.img_process(image_feq, mask, self.fundusAug)


        # image_new_feq = hfc_mul_mask(self.hfc_filter_in, image, mask, do_norm=True)
        # # print(image_new_feq.shape)
        # image_new_feq = self.img_process_freq(image_new_feq, mask, self.fundusAug)
        ...
        with autocast():  #### 
            ...
            features_new,features_new_distill = self.network(image_new,feat=True)
            ...
            # patch_options = [16, 8, 4]
            
            # random_number = np.random.randint(1, len(patch_options))
            
            # image_new_patch_mix = self.patchify(image_new, block=patch_options[random_number])
            # image_new_patch_mix = self.random_patch_mix(image_new_patch_mix, self.cfg.MASK_RATIO)
            # image_new_patch_mix = self.unpatchify(image_new_patch_mix,patch_options[random_number])
            # image_ori_patch_mix  = self.patchify(image_ori, block=patch_options[random_number])
            # image_ori_patch_mix = self.random_patch_mix(image_ori_patch_mix, self.cfg.MASK_RATIO)
            # image_ori_patch_mix = self.unpatchify(image_ori_patch_mix,patch_options[random_number])
                
            # features_ori_patch_mix = self.network(image_ori_patch_mix)

            # features_new_patch_mix = self.network(image_new_patch_mix,drop_rate=0.3)
                
            
            # image_new_masked_feq=self.mask_freq.transform(image_new)
            # image_new_masked_feq=image_new_masked_feq.cuda()
            
            
            
            # features_new_masked = self.network(image_new_masked,drop_rate=0.5)
            features_new_masked,feat_new_masked_distill = self.network(image_new_masked,drop_rate=0.3, feat=True)
            features_new_masked_c,feat_new_masked_distill_c = self.network(image_new_masked,feat=True)

            # [self.x1, self.x2, self.x3, self.x4]= feat_new_masked_distill
            ...
            # feat_new_masked_distill1= self.align1(feat_new_masked_distill[0])
            # feat_new_masked_distill2= self.align2(feat_new_masked_distill[1])
            # feat_new_masked_distill3= self.align3(feat_new_masked_distill[2])

            # # for  i in range (3):
                
            # # print(features_new_distill[3].shape)
            # f_t1 = F.adaptive_avg_pool2d(feat_new_masked_distill1, (7, 7))
            
            # f_t2 = F.adaptive_avg_pool2d(feat_new_masked_distill2, (7, 7))

            # f_t3 = F.adaptive_avg_pool2d(feat_new_masked_distill3, (7, 7))


            # # feat_new_masked_distill[i] = self.align1(feat_new_masked_distill[i])
            # loss_feat += loss_mse(f_t1, features_new_distill[3].detach())
            
            # loss_feat += loss_mse(f_t2, features_new_distill[3].detach())

            # loss_feat += loss_mse(f_t3, features_new_distill[3].detach())
            ...
            output_new_masked1 = self.classifier1(features_new_masked[0])
            output_new_masked2 = self.classifier2(features_new_masked[1])
            output_new_masked3 = self.classifier3(features_new_masked[2])
            
            
            output_new_masked_c1 = self.classifier(features_new_masked_c[3])

            output_new_masked_c1 = self.classifier1(features_new_masked_c[0])
            output_new_masked_c2 = self.classifier2(features_new_masked_c[1])
            output_new_masked_c3 = self.classifier3(features_new_masked_c[2])
            ...
            # z_new_masked = self.projector(features_new_patch_mix)
            # z_ori_masked = self.projector(features_ori_patch_mix)
                
            # p_new_masked, p_ori_masked = self.predictor(z_new_masked), self.predictor(z_ori_masked)
                
                
            # z_new_masked, z_ori_masked = nn.functional.normalize(z_new_masked, dim=-1), nn.functional.normalize(z_ori_masked, dim=-1)
            # p_new_masked, p_ori_masked = nn.functional.normalize(p_new_masked, dim=-1), nn.functional.normalize(p_ori_masked, dim=-1)
            ...
            if self.cfg.FASTMOCO > 0:
                ...
                # r = np.random.rand(1)
                # if r < 0.5:
                #     x1_in_form = self._local_split(image_ori_patch_mix)
                #     x2_in_form = self._local_split(image_new_patch_mix)
                # else:
                # x1_in_form = self._local_split(self.hp_transform(image_ori))   
                # x2_in_form = self._local_split(self.hp_transform(image_new))
                ...
                loss_fastcoco, loss_dict_iter = self.criterion([output_new, output_ori, output_new_masked], [features_ori, features_new,features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix, p2_orthmix, features_x_mixed, mix_z_, mix_result,targets_b,lam],  label, domain)

                temperature=2.0
                
                # tau1, tau2, tau3, tau=-0.3, 0.5, 1.5, 1.0

                # if self.cfg.DATASET.SOURCE_DOMAINS[0]   # 为不同的数据设置不同的参数
                tau1, tau2, tau3, tau=-0.2, 1.0, 1.5, 0.55

                self.SupLoss3 = LogitAdjust(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau=tau3)    #### 使用逻辑补偿loss
                ...
                TEMP=0.5
                loss3_sup += TEMP * self.SupLoss3(output_new3, label)
                loss2_sup += TEMP * self.SupLoss2(output_new2, label)
                loss1_sup += TEMP * self.SupLoss1(output_new1, label)
                
                loss_sup += TEMP * self.SupLoss(output_new, label)

                # print(loss_fastcoco, loss1_sup, loss2_sup, loss3_sup, loss_sup )

                # self.SupLoss3 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau3,tau2=tau3)    #### 使用逻辑补偿loss
                ...
                # TEMP= self.cfg.GDRNET.MASKED * min( self.epoch / 25, 1.0)
                
                # loss3_sup += TEMP * self.SupLoss3(output_new_masked3, output_new3, label)
                # loss2_sup += TEMP * self.SupLoss2(output_new_masked2, output_new2, label)
                # loss1_sup += TEMP * self.SupLoss1(output_new_masked1, output_new1, label)
                
                # loss_sup += TEMP * self.SupLoss(output_new_masked, output_new, label)  ### 
 
 
 
                self.SupLoss3 = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau=tau3)    #### 使用逻辑补偿loss
                ...
                # loss3_sup += TEMP * self.SupLoss3(output_new_masked_c3, output_new)
                # loss2_sup += TEMP * self.SupLoss2(output_new_masked_c2, output_new)
                # loss1_sup += TEMP * self.SupLoss1(output_new_masked_c1, output_new)
                
                # loss_sup += TEMP * self.SupLoss(output_new_masked, output_new)  ### 
                


                loss_sup += - 0.5  * (F.softmax(output_new / temperature, 1).detach() * F.log_softmax(output_new_masked / temperature, 1)).sum() / output_new1.size()[0]



                tau1, tau2, tau3, tau=0.0, 0.0, 0.0, 0.0

                # self.SupLoss3 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau3,tau2=tau)    #### 使用逻辑补偿loss
                # self.SupLoss2 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau2,tau2=tau)    #### 使用逻辑补偿loss
                # self.SupLoss1 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau1,tau2=tau)    #### 使用逻辑补偿loss
                # self.SupLoss = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau3,tau2=tau3)    #### 使用逻辑补偿loss
                
                
                # TEMP= self.cfg.GDRNET.MASKED  * min( self.epoch / 25, 1.0) #* 2

                # loss3_sup += TEMP * self.SupLoss3(output_new3, output_new, label)
                # loss2_sup += TEMP * self.SupLoss2(output_new2, output_new, label)
                # loss1_sup += TEMP * self.SupLoss1(output_new1, output_new, label)
                
 
                self.SupLoss3 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau,tau2=tau3)    #### 使用逻辑补偿loss
                self.SupLoss2 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau,tau2=tau2)    #### 使用逻辑补偿loss
                self.SupLoss1 = LogitAdjust_KD_V2(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau,tau2=tau1)    #### 使用逻辑补偿loss
                # self.SupLoss = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau1=tau3,tau2=tau3)    #### 使用逻辑补偿loss
                ...
                # output_new1_softmax = F.softmax((output_new1) / temperature, dim=1).detach()
                # output_new2_softmax = F.softmax((output_new2) / temperature, dim=1).detach()
                # output_new3_softmax = F.softmax((output_new3) / temperature, dim=1).detach()

                # output_new_softmax = F.softmax((output_new) / temperature, dim=1).detach()


                # output_new1_max, output_new1_index = torch.max(F.softmax((output_new1), dim=1).detach(), dim=1)
                # output_new2_max, output_new2_index = torch.max(F.softmax((output_new1), dim=1).detach(), dim=1)
                # output_new3_max, output_new3_index = torch.max(F.softmax((output_new1), dim=1).detach(), dim=1)
                ...
                # loss += TEMP * F.kl_div(output_new_softmax[(output_new1_index == label)],
                #                         output_new1_softmax[(output_new1_index == label)],
                #                          reduction='batchmean') * (temperature ** 2)
                
                # loss += TEMP *F.kl_div(output_new_softmax[(output_new2_index == label)],
                #                         output_new2_softmax[(output_new2_index == label)],
                #                          reduction='batchmean') * (temperature ** 2)
                
                # loss += TEMP * F.kl_div(output_new_softmax[(output_new3_index == label)],
                #                         output_new3_softmax[(output_new3_index == label)],
                #                          reduction='batchmean') * (temperature ** 2)
                ...
                # self.SupLoss3 = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau=0.9)    #### 使用逻辑补偿loss
                # self.SupLoss2 = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau=0.3)    #### 使用逻辑补偿loss
                # self.SupLoss1 = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau=0.0)    #### 使用逻辑补偿loss
                # self.SupLoss = LogitAdjust_KD(self.label_num_dict[self.cfg.DATASET.SOURCE_DOMAINS[0]],tau=0.5)    #### 使用逻辑补偿loss
                ...
                # loss += TEMP * self.SupLoss1(output_new_masked1, output_new1.detach())  ### 知识蒸馏
                ...
                # loss = loss_fastcoco + loss1_sup + loss2_sup + loss3_sup + loss_sup # + 0.000001 * loss_feat
                
                loss = loss_fastcoco + loss_sup # + 0.000001 * loss_feat
                ...
                # loss_dict_iter['loss_siam_fastmoco'] = loss.item()
    ...
    def predict(self, x):
        ...
        output1 = self.classifier1(features[0])
        output2 = self.classifier2(features[1])
