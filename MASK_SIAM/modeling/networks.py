...
# [ICCV 2023] Cross Contrasting Feature Perturbation for Domain Generalization (ICCV'23)
...
class ResNet(torch.nn.Module):
    ...
    def forward(self, x,drop_rate=0.0):
        ...
        x = self.network.fc(x)
        if drop_rate > 0.0:
            output = F.dropout(output, p=float(drop_rate))
        return output
...
class ResNet_Distill(torch.nn.Module):
    """ResNet with the softmax chopped off and the batchnorm frozen"""
    def __init__(self, cfg):
        super(ResNet_Distill, self).__init__()
        ...
        self.dropout = nn.Dropout(0.3)
        self.norm = DistributionUncertainty()
        self.p_norm = TriD(p=0.5) #PermuteAdaptiveInstanceNorm2d()
        # self.p_norm = PermuteAdaptiveInstanceNorm2d()  ###

        # self.align1 = nn.Conv2d(student_channels, teacher_channels, kernel_size=1, stride=1, padding=0)
        # self.align2 = nn.Conv2d(student_channels, teacher_channels, kernel_size=1, stride=1, padding=0)
        # self.align3 = nn.Conv2d(student_channels, teacher_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, drop_rate=0.0,  perturb=True, feat=False):
        """Encode x into a feature vector of size n_outputs."""
        x = self.network.conv1(x)
        # if perturb:
        #     # x = self.norm(x)
        ...
        if perturb:
            ...
            x = self.p_norm(x)
        self.x1 = x
        x1 = self.network.avgpool(self.x1)

        x1 = x1.view(x1.size(0), x1.size(1))
        x1 = self.network.fc(x1)
        ...
        x2 = self.network.avgpool(self.x2)
        x2 = x2.view(x2.size(0), x2.size(1))
        x2 = self.network.fc(x2)
        ...
        self.x3 = x
        x3 = self.network.avgpool(self.x3)
        x3 = x3.view(x3.size(0), x3.size(1))
        x3 = self.network.fc(x3)
        output3 = self.dropout(x3)
        ...
        if feat:
                return [output1, output2, output3, output],[self.x1, self.x2, self.x3, self.x4]
            
        return output #[output1, output2, output3, output]
...
class ResNet_tea(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.norm0 = LDP(cfg, 64)
        self.norm1 = LDP(cfg, 64)
        self.norm2 = LDP(cfg, 256)
        self.norm3 = LDP(cfg, 512)
        self.norm4 = LDP(cfg, 1024)
        self.norm = DistributionUncertainty()
        self.p_norm = PermuteAdaptiveInstanceNorm2d()
