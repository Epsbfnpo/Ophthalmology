...
from modeling.losses import DahLoss,DahLoss_Dual, DahLoss_Dual_BalSCL,DahLoss_Mask, DahLoss_Dual_Siam, DahLoss_Siam,DahLoss_Siam_Fastmoco,DahLoss_Siam_Fastmoco_v0_bsda
...
from sam import SAM
from modeling.isda_warp import ISDALoss
from modeling.bsda_warp import BSDALayer
...
class GDRNet_MASK_SIAM(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        self.brsda_lambda = 0.8
        self.brsda_alpha = 0.8
        self.multi = 1
        self.bsda_lambda = 0.8
        self.bsda_alpha = 0.5
        self.brsda_kl_weight = 5e-5
        self.brsda_recon_weight = 1.0
        # self.brsda_layer = BSDALayer(self.network.out_features(), self.brsda_lambda)
        # self.isda_criterion = ISDALoss(self.network.out_features() , cfg.DATASET.NUM_CLASSES) #.cuda()
        
        
        self.brsda_layer = BSDALayer(self.network.n_outputs, self.brsda_lambda)
        self.isda_criterion = ISDALoss(self.network.n_outputs, cfg.DATASET.NUM_CLASSES) #.cuda()
        ...
        init_weights_MLPHead(self.brsda_layer, init_method='He')
        ...
        self.optimizer = torch.optim.Adam(
            ...
            {"params":self.brsda_layer.parameters(), 'fix_lr': False},     ### 加入特征语义增强
        ...
        if self.cfg.FASTMOCO > 0:
            self.split_num = 2
            self.combs = 3
            self.criterion =  DahLoss_Siam_Fastmoco_v0_bsda(cfg=self.cfg, beta= cfg.GDRNET.BETA, max_iteration = cfg.EPOCHS, \
                                    training_domains = cfg.DATASET.SOURCE_DOMAINS, temperature = cfg.GDRNET.TEMPERATURE, \
    ...
    def update(self, minibatch):
        ...
        image_new, image_ori, image_all = self.img_process(image, mask, self.fundusAug)
        ...
        with autocast():  #### 
            ...
            features_ori = self.network(image_ori)
            features_new = self.network(image_new,  perturb=True)
            ...
            m, mu, logvar, a_hat = self.brsda_layer(features_new, multi=self.multi)  ### a_hat:重构的features_new
            a_tilde = self.brsda_layer.calc_a_tilde(features_new, m, multi=self.multi) ### a_tilde:增强的features_new
            y_hat_tilde = self.classifier(a_tilde)  ### 预测输出
            
            # domain_weight, class_weight = self.criterion.get_weights(label, domain)
            
            # features_new_isda = self.isda_criterion(features_new, output_new, self.classifier.parameters(), label, 0.5)  ## 增强后的预测输出
            
            
            loss_task_tilde = self.criterion2(y_hat_tilde, label.repeat(self.multi, ))   ### 重构的loss
            
            
            # loss_task_tilde = torch.mean(loss_task_tilde * class_weight * domain_weight) / (torch.mean(domain_weight) * torch.mean(class_weight)) 
            # print(y_hat_tilde.shape, label.repeat(self.multi, ).shape)
        
            loss_brsda_kl = self.brsda_layer.calc_kl_loss(mu, logvar)
            loss_brsda_recon = self.brsda_layer.calc_recon_loss(features_new, a_hat, self.multi)
            loss_brsda = loss_brsda_kl * self.brsda_kl_weight + loss_brsda_recon * self.brsda_recon_weight # + 0.25 * loss_task_tilde
            
            # print('loss_brsda_kl', loss_brsda_kl, 'loss_brsda_recon', loss_brsda_recon, 'loss_task_tilde', loss_task_tilde)

            features_new = a_tilde 
            ...
            features_new_masked = self.network(image_new_masked,drop_rate=0.3,  perturb=True)
            ...
            if self.cfg.FASTMOCO > -1.0:
                ...
                # print(loss_brsda_kl, loss_brsda_recon, loss_task_tilde)
                loss, loss_dict_iter = self.criterion([output_new, y_hat_tilde, output_new_masked], [features_ori, features_new, features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix, p2_orthmix, z_all_orthmix, p_all_orthmix, features_all_z, z_all_sup, p_all],  [z_new_masked, z_ori_masked_c, p_new_masked, p_ori_masked_c], label, domain, self.random_matrix)



                # loss += loss_brsda_kl * self.brsda_kl_weight + loss_brsda_recon * self.brsda_recon_weight + loss_task_tilde * self.task_weight  #+ loss_ctr * self.ctr_weight
            ...
            #     loss, loss_dict_iter = self.criterion([output_new, output_new_masked], [features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2], label, domain)
                ...
                loss += loss_brsda # (loss_task_tilde+ loss_brsda) * self.brsda_alpha + loss_task
