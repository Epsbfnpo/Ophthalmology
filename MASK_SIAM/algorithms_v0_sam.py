...
class GDRNet_MASK_SIAM(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        K= 1024 # 1536 # 1024
        dim= 512 # 2048  #################### 此处应与projector保持一致
        self.K=K
        self.num_positive = 0
    ...
    def update(self, minibatch):
        ...
        with autocast():  #### 
            ...
            features_ori_z1 = self.projector(features_ori)  ## z1
            ...
            features_new_masked = self.network(image_new_masked,drop_rate=0.3)
            ...
            if self.cfg.FASTMOCO > 0:
                ...
                loss, loss_dict_iter = self.criterion([output_new, output_ori, output_new_masked], [features_ori, features_new, features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix, p2_orthmix, z_all_orthmix, p_all_orthmix, features_all_z, z_all_sup, p_all],  label, domain)
...
# features_ori, features_new,features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix, p2_orthmix, features_x_mixed, mix_z_, mix_result, targets_b,lam
        ...
        # amp_grad_scaler.step(self.optimizer.first_step(zero_grad=True))
...
# features_ori, features_new,features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix, p2_orthmix, features_x_mixed, mix_z_, mix_result, targets_b,lam
        ...
        # amp_grad_scaler.scale(loss).backward()
        # amp_grad_scaler.step(self.optimizer.second_step(zero_grad=True))
        # # amp_grad_scaler.step(self.optimizer1)
        # amp_grad_scaler.update()
