...
class CASS_GDRNet(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        self.vit_head_params = [p for p in vit_params_all if 'classifier_vit' in id_to_name.get(id(p), '') or 'projector_vit' in id_to_name.get(id(p), '') or 'predictor_vit' in id_to_name.get(id(p), '')]
        self.vit_lora_params = [p for p in vit_params_all if 'classifier_vit' not in id_to_name.get(id(p), '') and 'projector_vit' not in id_to_name.get(id(p), '') and 'predictor_vit' not in id_to_name.get(id(p), '')]
        self.opt_cnn = torch.optim.AdamW(self.cnn_params, lr=1e-4, weight_decay=1e-4)
        self.opt_vit = torch.optim.AdamW([{'params': self.vit_lora_params, 'lr': self.base_lr_vit, 'weight_decay': 0.05}, {'params': self.vit_head_params, 'lr': self.base_lr_vit_head, 'weight_decay': 1e-4},], betas=(0.9, 0.999),)
        ...
        assert self.K % (batch_size_per_gpu * world_size) == 0, (f"队列容量 K ({self.K}) 必须被全局 Batch Size ({batch_size_per_gpu * world_size}) 整除！")
        ...
        self.cnn_train_transforms = v2.Compose([v2.RandomResizedCrop(224, scale=(0.6, 1.0), antialias=True), v2.RandomHorizontalFlip(p=0.5), v2.RandomVerticalFlip(p=0.5), v2.RandomRotation(45), v2.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1), v2.RandomApply([v2.GaussianBlur(kernel_size=5)], p=0.3), v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
        self.vit_train_transforms = v2.Compose([v2.RandomHorizontalFlip(p=0.5), v2.RandomRotation(10), v2.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.05), v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
        self.weak_transforms = v2.Compose([v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
        self.weak_transforms_cnn = v2.Compose([v2.Resize((224, 224), antialias=True), v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
    ...
    def update(self, minibatch):
        ...
        with autocast_ctx():
            res_combined = self.network(x_cnn=img_strong_cnn, x_vit=img_strong_vit, return_train_features=True)
        res_clean_fp32 = {'proj_cnn': res_combined['proj_cnn'].float(), 'proj_vit': res_combined['proj_vit'].float(), 'pred_cnn': res_combined['pred_cnn'].float(), 'pred_vit': res_combined['pred_vit'].float(), 'logits_cnn': res_combined['logits_cnn'].float(), 'logits_vit': res_combined['logits_vit'].float(),}
        with torch.no_grad():
            momentum_prev_mode = momentum_inner.training
            momentum_inner.eval()
            with autocast_ctx():
                res_momentum = momentum_inner(x_cnn=img_weak_cnn, x_vit=img_weak_vit, return_train_features=True)
        ...
        loss_main = loss_sup
        loss_dict = {'loss': loss_main.item(), 'sup_cnn': loss_sup_cnn.item(), 'sup_vit': loss_sup_vit.item(), 'loss_contrastive': loss_contrastive.item(), 'loss_instance': loss_instance.item(), 'loss_grade': loss_grade.item(), 'lambda_grade': lambda_grade,}
        loss_dict.update({'loss_inst_cnn2vit': loss_inst_cnn2vit.item(), 'loss_inst_vit2cnn': loss_inst_vit2cnn.item(), 'loss_grade_cnn2vit': loss_grade_cnn2vit.item(), 'loss_grade_vit2cnn': loss_grade_vit2cnn.item(),})
        kd_temp = 2.0
        with torch.no_grad():
