...
def load_checkpoint(path, algorithm, optimizer, scheduler):
    ...
    # 注意：如果使用了新的 Scheduler，旧的 state_dict 可能会报错，这里加个简单的 try-catch 保护
    try:
        scheduler.load_state_dict(checkpoint['scheduler_state'])
    except Exception as e:
        print(f"[Warning] Failed to load scheduler state (maybe scheduler type changed?): {e}")
...
def main():
    ...
    set_seed(cfg.SEED)  # [修改] 使用 cfg.SEED
