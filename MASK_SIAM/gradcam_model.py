...
class DG_Model(torch.nn.Module):
    def __init__(self, cfg, log_path):
        ...
        net_path = os.path.join(log_path, 'best_model_ema.pth')
        classifier_path = os.path.join(log_path, 'best_classifier_ema.pth')
...
def do_gradcam(minibatch, cfg, log_path, idx):  
    ...
    grayscale_cam = gradcam(input_tensor=image, targets=[ClassifierOutputTarget(label)])  ### 标签

    grayscale_cam_pred = gradcam(input_tensor=image, targets=[ClassifierOutputTarget(pred)])  ### 标签


    # In this example grayscale_cam has only one image in the batch:
    grayscale_cam = grayscale_cam[0, :]
    # print(image.shape)
    input_image_np = image.squeeze().permute(1, 2, 0).cpu().detach().numpy()  ## hwc
    np_image = input_image_np.astype(np.float32) / 255
    visualization = show_cam_on_image(np_image, grayscale_cam, use_rgb=False,image_weight=0.9)

    grayscale_cam_pred = grayscale_cam_pred[0, :]
    visualization_pred = show_cam_on_image(np_image, grayscale_cam_pred, use_rgb=True)
...
# overlay = (255 * cmap(np.asarray(overlay) ** 2)[:, :, :3]).astype(np.uint8) 
    ...
    im_pred = Image.fromarray(visualization_pred)
    im_pred.save(os.path.join(save_dir, f'heatmap_{idx}_pred.png'))
