...
# [Modified] 增强版 update_writer，支持打印所有 loss
def update_writer(writer, epoch, scheduler, loss_avg):
    # 获取 loss 字典均值
    loss_means = loss_avg.get_means()

    # 构造日志字符串
    loss_str = ", ".join([f"{k}: {v:.4f}" for k, v in loss_means.items()])
    logging.info(f'epoch: {epoch}, {loss_str}')

    # 写入 TensorBoard
    writer.add_scalar('info/lr', scheduler.get_last_lr()[0], epoch)
    for k, v in loss_means.items():
        writer.add_scalar(f'train_loss/{k}', v, epoch)
...
class LossCounter:
    ...
    def update(self, loss_data):
        if isinstance(loss_data, (float, int, torch.Tensor)) and not isinstance(loss_data, dict):
            loss_data = {'loss': loss_data}
    ...
    def mean(self, key='loss'):
        if self.iteration == 0: return 0.0
        return self.loss_sums.get(key, 0.0) / self.iteration

    def get_means(self):
        if self.iteration == 0: return {k: 0.0 for k in self.loss_sums}
        return {k: v / self.iteration for k, v in self.loss_sums.items()}