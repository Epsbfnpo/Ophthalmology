...
class GDRNetLoss_Integrated(nn.Module):
    ...
    def forward(self, output_dict, labels, domains):
        ...
        if 'p_cnn' in output_dict and 'proj_vit' in output_dict:
            p_cnn = output_dict['p_cnn']
            z_vit = output_dict['proj_vit'].detach()
            loss_feat_distill = - F.cosine_similarity(p_cnn, z_vit, dim=-1)
            loss_feat_distill = (loss_feat_distill * dcr_weight).mean()
        else:
            loss_feat_distill = torch.tensor(0.0).to(logits_cnn.device)
        loss_total = loss_sup_vit + 0.2 * loss_sup_cnn + 1.0 * loss_distill_cnn + 1.0 * loss_feat_distill
        loss_dict = {"loss": loss_total.item(), "sup_vit": loss_sup_vit.item(), "sup_cnn": loss_sup_cnn.item(), "distill_cnn": loss_distill_cnn.item(), "feat_distill": loss_feat_distill.item()}
