...
# =========================================================================
# 2. 修改后的 FrozenViT (移除 Flash Attention，保留 Gradient Checkpointing)
# =========================================================================
class FrozenViT(PreTrainedModel):
    ...
    def from_pretrained(cls, model_path, use_lora=True, lora_rank=8, **kwargs):
        ...
        # [Revert] 移除了强制 Flash Attention 2 的代码，避免底层兼容性报错

        model = cls(config)

        print(f"Loading ViT Backbone from: {model_path}")
        # [Revert] 移除了 torch_dtype=torch.bfloat16，避免类型冲突，后续推荐使用 AMP 管理精度
        ...
        # [Optimization] 梯度检查点 (Gradient Checkpointing)
        # 这是一个通用的 PyTorch 特性，不依赖 Flash Attention，强烈建议保留
        if use_lora:
            print(f">> [Optimization] Enabling Gradient Checkpointing for LoRA training...")
            # 部分旧版本 transformers 可能需要 use_reentrant=False 来避免报错，但通常直接调用即可
