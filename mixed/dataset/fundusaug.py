...
levels = {'SH': [0, 2], 'HS': [[0, 2], [1, 1.2, 1.5, 2], [0.8, 0.1, 0.05, 0.05]], 'HG': [[0.4, 0.9], [-0.06, -0.01]],
          'SP': [[1, 8], [0.01, 0.08]], 'IB': [0.1, 3]}
...
class Compose(object):
    ...
    def __call__(self, image, mask=None):
        ...
        # [Critical Fix] 如果输入没有 mask，则只返回 image
        # 这样才能兼容 torchvision.transforms.Compose
        if mask is None:
            return image
...
class Halo(object):
    def __init__(self, size, brightness_factor=None, prob=0.5):
        self.size = size
        # [Fix] Removed .cuda()
        self.zeros = torch.zeros((self.size, self.size))
    ...
    def __call__(self, image, target=None):
        if random.random() < self.prob:
            # [Fix] Convert PIL to Tensor if needed
            is_pil = not isinstance(image, torch.Tensor)
            if is_pil:
            ...
            w0_a = random.randint(int(self.size / 2) - int(self.size / 8), int(self.size / 2) + int(self.size / 8))
            center_a = [w0_a, w0_a]
            ...
            mask_a[circle_a] = torch.mean(image)
            ...
            dia_b_max = 2 * int(np.sqrt(
                max(center_a[0], self.size - center_a[0]) * max(center_a[0], self.size - center_a[0]) + max(center_a[1],
                                                                                                            self.size -
                                                                                                            center_a[
                                                                                                                1]) * max(
                    center_a[1], self.size - center_a[1]))) / self.size
            wei_dia_b = 1.0 + (dia_b_max - 1.0) * random.random()

            if num == 0:
                dia_b = self.size * wei_dia_b + abs(
                    max(center_b[0] - self.size / 2, center_b[1] - self.size / 2) + self.size * 2 / 3)
            else:
                dia_b = self.size * wei_dia_b + abs(
                    max(center_b[0] - self.size / 2, center_b[1] - self.size / 2) + self.size / 2)
            ...
            delta_circle = F.gaussian_blur(torch.reshape(delta_circle, (1, self.size, self.size)),
                                           (gauss_rad, gauss_rad), sigma)

            if num == 0 or num == 1 or num == 2:
                delta_circle = torch.stack(
                    [(weight_r[num] * delta_circle), (weight_g[num] * delta_circle), (weight_b[num] * delta_circle)])
            else:
                num = 1
                delta_circle = torch.stack(
                    [(weight_r[num] * delta_circle), (weight_g[num] * delta_circle), (weight_b[num] * delta_circle)])
            ...
            # [Fix] Convert back to PIL if input was PIL
            if is_pil:
...
class Hole(object):
    ...
    def __call__(self, image, target=None):
        if random.random() < self.prob:
            ...
            diameter_circle = random.randint(int(self.min_diameter_circle * self.size),
                                             int(self.max_diameter_circle * self.size))
            center = [random.randint(int(self.size / 4), int(self.size * 3 / 4)),
                      random.randint(int(self.size * 3 / 8), int(self.size * 5 / 8))]
            ...
            # Safety check for target validation could be added here
            num_valid = torch.sum(target) if target is not None else 1
            # Assuming mean of image if target unavailable or complex
            aver_color = torch.mean(image)
    ...
    def __call__(self, image, target=None):
        if random.random() < self.prob:
            ...
            for i in range(s_num):
                ...
                center = [random.randint(radius + 1, self.size - radius - 1),
                          random.randint(radius + 1, self.size - radius - 1)]
                Y, X = np.ogrid[:self.size, :self.size]
                dist_from_center = np.sqrt((X - center[0]) ** 2 + (Y - center[1]) ** 2)
                ...
                mask = self.zeros.clone()
                # [Fix] Removed .cuda()
                mask[circle] = torch.multiply(A[0], torch.tensor(1 - t))
                mask0 = mask0 + mask
                mask0[mask0 != 0] = 1
