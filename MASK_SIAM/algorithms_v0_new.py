...
from utils.validate import algorithm_validate, algorithm_validate1
import modeling.model_manager as models
from modeling.losses import DahLoss,DahLoss_Dual, DahLoss_Dual_BalSCL,DahLoss_Mask, DahLoss_Dual_Siam, DahLoss_Siam,DahLoss_Siam_Fastmoco,DahLoss_Siam_Fastmoco_v0,DahLoss_Siam_Fastmoco_Dual
...
class GDRNet_MASK_SIAM(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        K= 1024 # 1536 # 1024  
        dim = 2048 # 2048  #################### 此处应与projector保持一致
        self.K=K
        self.num_positive = 64 #64
    ...
    def sample_target(self, features, labels):  ### 只有当前的labele
        ...
        for i, label in enumerate(labels):
            pos = torch.where(self.queue_labels == label)[0]   ### 与当前label相同的样本
            if len(pos) != 0:
        ...
        print(features.shape, labels.shape, neighbor.shape,)

        return neighbor
        
    @torch.no_grad()
    def sample_target_top_k(self, features, labels):  ### 只有当前的labele
        neighbor = []
        ...
        for i, label in enumerate(labels):
            ...
            if len(pos) != 0:
                if self.num_positive > 0:
                    choice = torch.multinomial(torch.ones_like(pos).type(torch.FloatTensor), self.num_positive, replacement=True)    ### replacement ：默认值值是False，即不放回采样。如果replacement =False，则
                    idx = pos[choice]
                    neighbor.append(self.queue[idx].mean(0))
                else:
                    neighbor.append(self.queue[pos].mean(0))  ###
    ...
    def update(self, minibatch):
        ...
        with autocast():  #### 
            ...
            # features_all_z = self.projector(features_all)
            ...
            image_ori_masked_c_ = self.patchify(image_new, block=self.cfg.BLOCK)
            ...
            features_ori_masked_c = self.network(image_ori_masked_c, drop_rate=0.3, perturb=False, distill=True)



            output_new_masked = self.classifier(features_new_masked[3])
            output_ori_masked_c = self.classifier(features_ori_masked_c[3])
            ...
            z_ori_masked_c = self.projector(features_ori_masked_c[3])
            ...
            if self.cfg.FASTMOCO > -1.0:
                ...
                # loss, loss_dict_iter = self.criterion([output_new, output_new_masked], [features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix,p1_orthmix, p2_orthmix], label, domain)d
                ...
                temp = self.cfg.KD
                # features_new_masked = features_ori_masked_c
                loss += temp * self.E_dis(features_new_masked[3], features_new_ema[3])  # .detach()# features_new_masked, features_new.detach()
                loss += temp * self.E_dis(features_new_masked[2], features_new_ema[2])  # features_new_masked, features_new.detach()
                loss += temp * self.E_dis(features_new_masked[1], features_new_ema[1])  # features_new_masked, features_new.detach()
                loss += temp * self.E_dis(features_new_masked[0], features_new_ema[0])  # features_new_masked, features_new.detach()
    ...
    # features_ori, features_new,features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix, p2_orthmix, features_x_mixed, mix_z_, mix_result, targets_b,lam
                ...
                loss, loss_dict_iter = self.criterion([output_new, output_new_ema], [features_ori_z1, features_new_z2, z1_sup, z2_sup, p1, p2], label, domain)
    ...
    def update_ema_model(self, iters):
        # self.epoch = epoch
        # iters = epoch * len(trainloader_u) + i
        ema_ratio = 0.996 # min(1 - 1 / (iters + 1), 0.996)
    ...
    def predict(self, x):
        ...
        # return self.classifier_ema(self.network(x))
...
class GDRNet_DUAL_MASK(Algorithm):
    ...
    def update(self, minibatch):
        ...
        self.loss_norm = self.cfg.DIS * (self.simi_tea0 + self.simi_tea1 + self.simi_tea2 + self.simi_tea3 + self.simi_tea4)
...
class GDRNet_DUAL_MASK_SIAM(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        self.cfg = cfg
        self.network1, self.network2 = models.get_net(cfg)  ## ResNet_tea
        self.classifier1 = models.get_classifier(self.network1.n_outputs, cfg)
        ...
        # # # #         # build a 3-layer projector
        self.projector1 = nn.Sequential(nn.Linear(dim_in1, feat_dim1, bias=False), 
                                    #    nn.BatchNorm1d(feat_dim1),
                                    nn.LeakyReLU(inplace=True), 
                                    nn.Linear(feat_dim1, dim_in1, bias=False),
                                    # nn.BatchNorm1d(feat_dim1),
        ...
        # # # #         # build a 3-layer projector
        self.projector2 = nn.Sequential(nn.Linear(dim_in2, feat_dim2, bias=False), 
                                    #    nn.BatchNorm1d(feat_dim1),
                                    nn.LeakyReLU(inplace=True), 
                                    nn.Linear(feat_dim2, dim_in2, bias=False),
                                    # nn.BatchNorm1d(feat_dim1),
                                    # nn.ReLU(inplace=True), 
                                    # nn.Linear(feat_dim1, dim_in1, bias=False), 
                                    # nn.BatchNorm1d(dim_in1, affine=False)
        ...
        init_weights_MLPHead(self.projector1, init_method='He')
        init_weights_MLPHead(self.predictor1, init_method='He')
        init_weights_MLPHead(self.projector2, init_method='He')
        init_weights_MLPHead(self.predictor2, init_method='He')
        ...
        self.optimizer1 = torch.optim.Adam(
            ...
            lr = 0.00045, #cfg.LEARNING_RATE, # cfg.LEARNING_RATE,
            # momentum = cfg.MOMENTUM,
            # weight_decay = 0.0, #= cfg.WEIGHT_DECAY,
            weight_decay = 0.0001, #= cfg.WEIGHT_DECAY,
            # nesterov=True
            )
        ...
        self.optimizer_LD = torch.optim.Adam([
        # self.optimizer_LD = torch.optim.SGD([
            # {'params': self.network2.norm0.parameters(), 'fix_lr': False},
            # {'params': self.network2.norm1.parameters(), 'fix_lr': False},
            {'params': self.network2.norm2.parameters(), 'fix_lr': False},
            {'params': self.network2.norm3.parameters(), 'fix_lr': False},
            {'params': self.network2.norm4.parameters(), 'fix_lr': False},
            {'params': self.network2.norm5.parameters(), 'fix_lr': False}
            ],
            lr = cfg.LEARNING_RATE,
            # momentum = cfg.MOMENTUM,
            weight_decay  = 0.0001, #= cfg.WEIGHT_DECAY,
        ...
        dim2=2048
        self.K=K
        self.num_positive = self.cfg.POSITIVE
        ...
        if self.cfg.FASTMOCO > -1.0:
            ...
            if self.cfg.DG_MODE =='DG':
                ...
                self.combs = 3
        # create queue for keeping neighbor
        self.register_buffer("queue", torch.randn(K, dim))  ### 1024 2048
        self.register_buffer("queue2", torch.randn(K, dim2))  #### 512

        self.queue = nn.functional.normalize(self.queue, dim=-1)
        self.register_buffer("queue_labels", -torch.ones(K, dtype=torch.long))
        self.register_buffer("queue_ptr", torch.zeros(1, dtype=torch.long))
        self.register_buffer("queue_labels2", -torch.ones(K, dtype=torch.long))
        self.register_buffer("queue_ptr2", torch.zeros(1, dtype=torch.long))

        self.fundusAug = get_post_FundusAug(cfg)
        self.criterion =  DahLoss_Dual_Siam(cfg, beta= cfg.GDRNET.BETA, max_iteration = cfg.EPOCHS, \
                                training_domains = cfg.DATASET.SOURCE_DOMAINS, temperature = cfg.GDRNET.TEMPERATURE, \
                                scaling_factor = cfg.GDRNET.SCALING_FACTOR)
    ...
    @torch.no_grad()
    def dequeue_and_enqueue2(self, features, labels):
        # gather features and labels before updating queue
        # features = concat_all_gather(features)
        # labels = concat_all_gather(labels)
        ...
        # replace the keys at ptr (dequeue and enqueue)
        self.queue2[ptr : ptr+batch_size, :] = features
        self.queue_labels2[ptr : ptr+batch_size] = labels
        ptr = (ptr + batch_size) % self.K  # move pointer
    ...
    @torch.no_grad()
    def sample_target_ori(self, features, labels):  ### 只有当前的labele
        neighbor = []
    ...
    def sample_target(self, features, labels):
        ...
        # for i, label in enumerate(labels):
        for z_i, label in zip(features, labels):  # z_i: [dim], 当前样本的投影特征（detach后）
            # 1. 找到队列中所有同类别正样本的索引
            pos = torch.where(self.queue_labels == label)[0]

            # cur_queue = self.queue.clone().detach()
            # sim_con_queue = features @ cur_queue

            # sim_con_pos = torch.gather(sim_con_queue, 1, pos[labels, :])

            if len(pos) != 0:
                if self.num_positive > 0:
                    
                    # 2. 取出所有同类别正样本 [M, dim]（M为队列中同类别样本数）
                    pos_samples = self.queue[pos]
                    # print(pos_samples.shape, self.queue.shape)  # torch.Size([116, 2048]) torch.Size([1024, 2048])
                    # 3. 计算当前样本与所有同类别样本的余弦相似度（点积=余弦相似度，因均归一化）
                    # z_i: [dim] -> 扩展为 [1, dim]，pos_samples: [M, dim]，点积后得到 [M]
                    similarities = torch.matmul(pos_samples, z_i.unsqueeze(dim=0).t()).squeeze(dim=1)
                    # print(similarities)
                    # 4. 选择相似度最低的top-k个难例正样本（k不超过同类别样本数M）
                    # k = pos_samples
                    # torch.topk(largest=False) 取最小值的top-k

                    if len(similarities) < self.num_positive:
                        # topk_neg_sim_indices = torch.topk(similarities, len(similarities), largest=False)[1]
                        # choice = torch.multinomial(torch.ones_like(pos).type(torch.FloatTensor), self.num_positive, replacement=True)    ### replacement ：默认值值是False，即不放回采样。如果replacement =False，则
                        choice = torch.multinomial(1-similarities, self.num_positive, replacement=True)    ### replacement ：默认值值是False，即不放回采样。如果replacement =False，则                       
                        # print(1-similarities)
                        # print(choice)
                        idx = pos[choice]
                        neighbor.append(self.queue[idx].mean(0))
                    else:
                        choice = torch.multinomial(1-similarities, self.num_positive, replacement=False)    
                        ...
                        # topk_neg_sim_indices = torch.topk(similarities, self.num_positive, largest=False)[1]
                        
                        # hard_pos_samples = pos_samples[topk_neg_sim_indices]  # [k, dim] 难例正样本
                    
                        # # 5. 难例正样本取平均，得到监督目标
                        # # neighbor_i = hard_pos_samples.mean(dim=0)  # [dim]
                        
                        # # choice = torch.multinomial(torch.ones_like(pos).type(torch.FloatTensor), self.num_positive, replacement=True)
                        # # idx = pos[choice]
                        # # neighbor.append(self.queue[idx].mean(0))
                        # neighbor.append(hard_pos_samples.mean(dim=0) )
                else:
                    neighbor.append(self.queue[pos].mean(0))
            else:
                # neighbor.append(features[i])
                neighbor.append(z_i)
        neighbor = torch.stack(neighbor, dim=0)
        # print(features.shape, neighbor.shape)  # torch.Size([32, 2048]) torch.Size([32, 2048])
        return neighbor

    @torch.no_grad()
    def sample_target2(self, features, labels):
        neighbor = []

        # for i, label in enumerate(labels):
        for z_i, label in zip(features, labels):  # z_i: [dim], 当前样本的投影特征（detach后）
            # 1. 找到队列中所有同类别正样本的索引
            pos = torch.where(self.queue_labels2 == label)[0]
            ...
            if len(pos) != 0:
                if self.num_positive > 0:
                    ...
                    if len(similarities) < self.num_positive:
                        ...
                        # print(choice)
                        idx = pos[choice]
                        neighbor.append(self.queue2[idx].mean(0))
                    else:
                        choice = torch.multinomial(1-similarities, self.num_positive, replacement=False)    
                    ...
                    neighbor.append(self.queue2[pos].mean(0))
            else:
                # neighbor.append(features[i])
                neighbor.append(z_i)
        neighbor = torch.stack(neighbor, dim=0)
    ...
    #                 # k = pos_samples
    #                 # torch.topk(largest=False) 取最小值的top-k
    #                 # print(similarities.shape,pos_samples.shape, z_i.shape )  # torch.Size([2]) torch.Size([2, 2048]) torch.Size([2048])
    #                 if len(similarities) < self.num_positive:
    #                     choice = torch.multinomial(torch.ones_like(pos).type(torch.FloatTensor), self.num_positive, replacement=True)    ### 根据给定权重对数组进行多次采样，返回采样后的元素下标。replacement ：默认值值是False，即不放回采样。如果replacement =False，则
    #                     idx = pos[choice]
    #                     neighbor.append(self.queue[idx].mean(0))
    ...
    def update(self, minibatch):
        ...
        features_ori2_z1 = self.projector2(self.network2(image_ori, perturb))  ## z1
        features_new2 = self.network2(image_new, perturb)
        ...
        z21_sup = self.sample_target2(features_ori2_z1.detach(), label)
        z22_sup = self.sample_target2(features_new2_z2.detach(), label)
        ...
        image_new_masked = self.unpatchify(masked * image_new_masked_,block=self.cfg.BLOCK)#block=self.cfg.BLOCK)
        image_ori_masked_c = self.unpatchify(masked_c * image_new_masked_, block=self.cfg.BLOCK)
        ...
        # l.normalize(mix_p22, dim=-1)

        # print(features_ori2_z1.shape, p21.shape )
        # print(mix_p11.shape,mix_new2_z2.shape ) # torch.Size([8, 2048]) torch.Size([8, 2048])
        # # # if self.cfg.MIXUP:
        # noise_std=0.05
        # index = torch.randperm(image_new.size(0)).cuda()
        # lam = np.random.beta(1.0, 1.0)
        ...
        # mix_result = self.classifier(mix_z)  + self.classifier(features_x_mixed[3])



        loss, loss_dict_iter = self.criterion([output_new1, output_new2, output_new_masked1, output_new_masked2], [features_ori1_z1,features_new1_z2, z11_sup, z12_sup, p11, p12, features_ori2_z1,features_new2_z2, z21_sup, z22_sup, p21, p22], None,  label, domain,self.random_matrix)

        # loss, loss_dict_iter = self.criterion([output_new1, output_new2, output_new_masked1, output_new_masked2], [features_ori1_z1,features_new1_z2, z11_sup, z12_sup, p11, p12, features_ori2_z1,features_new2_z2, z21_sup, z22_sup, p21, p22], [mix_ori1_z1, mix_new1_z2, mix_p11, mix_p12, mix_ori2_z1, mix_new2_z2, mix_p21, mix_p22], label, domain,self.random_matrix)

        loss += self.cfg.CONS * ( self.consis_stu +  self.consis_tea)
        ...
        # student
        _ = self.id_featurizer(image_new)
        fea_0, fea_1, fea_2, fea_3, fea_4, fea_5 = self.id_featurizer.x0, self.id_featurizer.x1, self.id_featurizer.x2, self.id_featurizer.x3, self.id_featurizer.x4, self.id_featurizer.x5
        # fea_0, fea_1 = self.id_featurizer.x0, self.id_featurizer.x1

        # teacher
        _ = self.id_featurizer_tea(image_new, perturb)
        fea_tea_0, fea_tea_1, fea_tea_2, fea_tea_3, fea_tea_4, fea_tea_5 = self.id_featurizer_tea.x0, self.id_featurizer_tea.x1, self.id_featurizer_tea.x2, self.id_featurizer_tea.x3, self.id_featurizer_tea.x4, self.id_featurizer_tea.x5
        # fea_tea_0, fea_tea_1 = self.id_featurizer_tea.x0, self.id_featurizer_tea.x1
        ...
        self.simi_tea3 = -self.F_distance(self.gram(fea_3.detach()), self.gram(fea_tea_3))
        self.simi_tea4 = -self.F_distance(self.gram(fea_4.detach()), self.gram(fea_tea_4))
        self.simi_tea5 = -self.F_distance(self.gram(fea_5.detach()), self.gram(fea_tea_5))

        self.loss_norm = self.cfg.DIS  * (self.simi_tea5  + self.simi_tea2 + self.simi_tea3 + self.simi_tea4)
        # print(self.loss_norm)
        # self.loss_norm = self.hparams['beta'] * (self.simi_tea0 + self.simi_tea1)
    ...
    def validate(self, val_loader, test_loader, writer):
        ...
        test_auc = -1
        if self.epoch <= self.cfg.EPOCHS:
            val_auc1, val_loss1 = algorithm_validate1(self, val_loader, writer, self.epoch, 'val')
            test_auc1, test_loss1 = algorithm_validate1(self, test_loader, writer, self.epoch, 'test')
            val_auc, val_loss = algorithm_validate(self, val_loader, writer, self.epoch, 'val')
            test_auc, test_loss = algorithm_validate(self, test_loader, writer, self.epoch, 'test')
            if self.epoch == self.cfg.EPOCHS:
                self.epoch += 1
    ...
    def renew_model(self, log_path):
        net_path = os.path.join(log_path, 'best_model.pth')
        classifier_path = os.path.join(log_path, 'best_classifier.pth')
        self.network2.load_state_dict(torch.load(net_path))
        self.classifier2.load_state_dict(torch.load(classifier_path))

    def predict(self, x):
        return self.classifier2(self.network2(x))
