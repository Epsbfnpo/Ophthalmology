...
class ERM(Algorithm):
    ...
    def __init__(self, num_classes, cfg):
        ...
        if 'FPT' in cfg.BACKBONE:
            ...
            print(f">> [Optimizer Info] LR: {cfg.LEARNING_RATE}, Weight Decay: {cfg.WEIGHT_DECAY}")
            ...
            print(f">> [Optimizer Info] Using standard SGD optimizer for {cfg.BACKBONE}.")
...
class GDRNet(ERM):
    ...
    def update(self, minibatch):
        self.global_step += 1
        debug = (self.global_step % 50 == 1)  # 每50步打印一次
        ...
        if 'FPT' in self.cfg.BACKBONE:
            logits_ori, proj_ori = self.network(image_ori)
            logits_new, proj_new = self.network(image_new)

            # [DEBUG] 打印 Logits 对应的预测类别
            if debug:
                pred_labels = logits_ori.argmax(dim=1)
                acc = (pred_labels == label).float().mean()
                print(f"[DEBUG Step {self.global_step}] Batch Acc: {acc.item():.4f}")
                print(f"[DEBUG] Label Sample: {label[:5].cpu().numpy()}")
                print(f"[DEBUG] Pred  Sample: {pred_labels[:5].cpu().numpy()}")

            loss, loss_dict_iter = self.criterion([logits_new], [proj_ori, proj_new], label, domain)
        else:
        ...
        # ---------------- [DEBUG] 梯度检查 ----------------
        if debug and 'FPT' in self.cfg.BACKBONE:
            ...
            for name, param in self.network.named_parameters():
                if param.grad is not None:
                    ...
                    has_grad = True
                    # 打印一些主要层的梯度，确保不是0
                    if 'side_encoder.encoder.layer.0' in name and 'weight' in name:
                        print(f"  -> SideViT Layer0 Grad Norm: {param_norm:.6f}")
                    if 'fusion_module' in name and 'weight' in name and param_norm > 0:
                        pass  # 确认有梯度即可

            print(f"  -> Total Grad Norm: {total_norm:.6f}")
            if not has_grad:
                print("  -> WARNING: No gradients found! Check requires_grad or computation graph.")
        # ----------------------------------------------------
