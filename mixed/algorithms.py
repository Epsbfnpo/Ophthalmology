...
class GDRNet(Algorithm):
    ...
    def update(self, minibatch):
        ...
        # [步骤D] OrthMix 正交混合 (核心修复：先混合!)
        # ！！！关键点！！！
        # 我们在 Backbone 输出的线性语义空间 (feat) 上进行混合
        # 而不是在 Projector 输出的非线性度量空间 (z) 上混合

        # 混合 Weak 流特征
        z1_orthmix_feat = torch.cat(
            list(map(lambda x: sum(x) / self.combs, list(combinations(feat_list_weak, r=self.combs)))),
            dim=0
        )

        # 混合 Strong 流特征
        z2_orthmix_feat = torch.cat(
            list(map(lambda x: sum(x) / self.combs, list(combinations(feat_list_strong, r=self.combs)))),
            dim=0
        )

        # [步骤E] Projector 投影 (后投影!)
        # 混合好的特征现在通过 Projector，学习如何将"混合语义"映射到目标空间
        z1_orthmix = self.projector(z1_orthmix_feat)
        z2_orthmix = self.projector(z2_orthmix_feat)

        # [步骤F] Predictor 预测
        # 对比学习的一端通常需要过 Predictor (SimSiam 结构)
        p1_orthmix = self.predictor(z1_orthmix)
        p2_orthmix = self.predictor(z2_orthmix)

        # [步骤G] 格式调整 (适配 Loss 接口)
        # DahLoss 需要 p_orthmix 是列表格式，而 z_orthmix 保持 Tensor 格式
        ...
        loss_main, loss_dict = self.criterion(
            ...
            [
                ...
                z1_orthmix,  # 修复后的 Weak 混合特征 (Tensor)
                z2_orthmix,  # 修复后的 Strong 混合特征 (Tensor)
                p1_orthmix_list,  # 修复后的 Weak 预测列表 (List)
                p2_orthmix_list  # 修复后的 Strong 预测列表 (List)
            ],
