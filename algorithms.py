...
class CASS_GDRNet(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        print(f"Total Params: {total_params / 1e6:.2f}M, Trainable: {trainable_num / 1e6:.2f}M")
    ...
    def saliency_guided_masking(self, x, imgs, mask_ratio=0.5, noise_weight=0.3):
        N, L, D = x.shape
        block_size = int((imgs.shape[2] * imgs.shape[3] / L) ** 0.5)
        ...
        sal_min = patch_saliency.min(dim=1, keepdim=True)[0]
        sal_max = patch_saliency.max(dim=1, keepdim=True)[0]
    ...
    def update(self, minibatch):
        ...
        x_cnn_input = self.unpatchify(x_cnn_masked_patch, block=block_size)
        with get_sync_context():
            with torch.amp.autocast('cuda'):
                res_clean = self.network(x_cnn=x_cnn_input, x_vit=x_vit_input)
        res_clean_fp32 = {'proj_cnn': res_clean['proj_cnn'].float(), 'proj_vit': res_clean['proj_vit'].float(), 'logits_cnn': res_clean['logits_cnn'].float(), 'logits_vit': res_clean['logits_vit'].float(), 'p_cnn': res_clean['p_cnn'].float()}
