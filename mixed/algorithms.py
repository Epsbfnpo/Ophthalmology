...
class GDRNet(Algorithm):
    ...
    def __init__(self, num_classes, cfg):
        ...
        # FastMoCo 相关参数 (恢复核心)
        self.split_num = 2  # 切分数量，2x2=4 块
        self.combs = 3  # 组合数

        # 2. Projector & Predictor (v0 结构)
        ...
        self.optimizer = torch.optim.Adam(
            [
                ...
                {"params": self.predictor.parameters(), 'fix_lr': True},  # Fix Predictor LR
            ],
        ...
        self.criterion = DahLoss_Siam_Fastmoco_v0(
            ...
            fastmoco=1.0  # 强制开启 FastMoCo
        )
        self.E_dis = nn.MSELoss()
        self.ce_loss = nn.CrossEntropyLoss()  # 用于 SPMix
    ...
    def update(self, minibatch):
        ...
        # GDRBench returns: img_weak, img_strong, img_mixed, mask, label1, label2, lam, domain, index

        all_data = minibatch
        img_weak = all_data[0].cuda()
        img_strong = all_data[1].cuda()

        # 检查是否包含 Mixed 数据 (根据返回长度)
        img_mixed = None
        if len(all_data) >= 9:
            ...
            # Fallback (Val/Test mode or incomplete return)
        ...
        feat_new = self.network(img_strong)['features']

        # C. 掩码视图前向传播
        # 注意：使用 FPT+ 的 features_list 用于蒸馏
        ...
        z1 = self.projector(feat_ori)  # Weak
        z2 = self.projector(feat_new)  # Strong
        ...
        # G. Supervised Targets (Queue)
        z1_sup = self.sample_target(z1.detach(), label)
        z2_sup = self.sample_target(z2.detach(), label)

        # ==========================================
        # 2. FastMoCo Split 逻辑 (恢复!)
        # ==========================================

        # 对 Strong Aug 图像进行切分并 Resize
        # 变成 (4*B, C, H, W)
        img_splits = self._local_split_and_resize(img_strong)

        # 输入网络提取特征
        # FPT+ 会认为这是 4*B 张图，输出 (4*B, 2048)
        feat_splits = self.network(img_splits)['features']

        # 分组: 将 4*B 还原为 List of [B, 2048]
        # split_size_or_sections = Batch Size
        feat_split_list = list(feat_splits.split(img_strong.size(0), dim=0))  # 长度为 4 的 list

        # OrthMix (Combinations)
        # 从 4 份中取 3 份进行组合 (Combinations)
        z_splits_list = [self.projector(f) for f in feat_split_list]  # 先 Project

        # 生成混合特征 z_orthmix
        z_orthmix = torch.cat(
            list(map(lambda x: sum(x) / self.combs, list(combinations(z_splits_list, r=self.combs)))),
            dim=0
        )  # (4*B, Dim)

        # Predict 混合特征
        p_orthmix = self.predictor(z_orthmix)

        # 切分回 list 传给 Loss (DahLoss 期望 list)
        p_orthmix_list = list(p_orthmix.split(img_strong.size(0), dim=0))

        # 注意：DahLoss 需要 z1_orthmix 和 z2_orthmix
        z2_orthmix = z_orthmix
        z1_orthmix = z_orthmix  # 暂时占位

        p1_orthmix_list = p_orthmix_list
        p2_orthmix_list = p_orthmix_list
        ...
        if img_mixed is not None:
            # 仅用于分类，不通过 Projector/Predictor
            logits_mixed = self.classifier(self.network(img_mixed)['features'])
            # Mixup Loss
            loss_spmix = lam * self.ce_loss(logits_mixed, label) + (1 - lam) * self.ce_loss(logits_mixed, label_mixed_b)
        ...
        loss_main, loss_dict = self.criterion(
            ...
            [feat_ori, feat_new, z1, z2, z1_sup, z2_sup, p1, p2, z1_orthmix, z2_orthmix, p1_orthmix_list,
             p2_orthmix_list],
            [z_new_masked, z_ori_masked_c, p_new_masked, p_ori_masked_c],
            label, domain, random_matrix=None
        )

        # Feature Consistency Loss (Distillation)
        loss_kd = 0.0
        ...
        # 从 feat_new 中获取解耦特征 (需要在 nets.py 中返回)
        # 假设 network 返回了 'tra_feat' 和 'dino_raw'
        out_new_full = self.network(img_strong)  # 重新获取一次完整输出
        ...
        loss_dict['loss_spmix'] = loss_spmix.item() if isinstance(loss_spmix, torch.Tensor) else 0
