...
def algorithm_validate(algorithm, data_loader, writer, epoch, val_type):
    ...
    if is_main_process:
        if writer is not None:
            ...
            writer.add_scalar('info/{}_qwk'.format(val_type), qwk, epoch)

        logging.info('{} - epoch: {}, loss: {:.4f}, acc: {:.4f}, auc: {:.4f}, F1: {:.4f}, QWK: {:.4f}.'.format(val_type, epoch, global_loss, acc, auc_ovo, f1, qwk))

    algorithm.train()
    return {'auc': auc_ovo, 'acc': acc, 'f1': f1, 'qwk': qwk, 'loss': global_loss}