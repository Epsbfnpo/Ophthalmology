    ...
    def __init__(self, num_classes, cfg):
        ...
        if 'FPT' in cfg.BACKBONE:
            ...
            self.optimizer = torch.optim.AdamW(
                ...
                lr = cfg.LEARNING_RATE,       # [修正] 直接读取全局 LR
                weight_decay = cfg.WEIGHT_DECAY # [修正] 直接读取全局 WD
            )
        else:
...
class GDRNet(ERM):
    ...
    def update(self, minibatch):
        ...
        # ---------------- [Critical Fix: Handle FPT+ Tuple Return] ----------------
        if 'FPT' in self.cfg.BACKBONE:
            # FPTPlusNet returns (logits, proj_features)
            # No need for self.classifier here as FPTPlusNet includes it
