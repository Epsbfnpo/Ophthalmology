...
class SideResNet(ResNet):
    ...
    def forward(self, x, dino_tokens, return_features=False):
        ...
        features_list.append(x)  # 存入 Stage 1 特征

        # Layer 2 + Fusion
        x = self.layer2(x)
        x = x + self.fusion2(dino_map, x.shape[-2:])
        features_list.append(x)  # 存入 Stage 2 特征

        # Layer 3 + Fusion
        x = self.layer3(x)
        x = x + self.fusion3(dino_map, x.shape[-2:])
        features_list.append(x)  # 存入 Stage 3 特征

        # Layer 4 + Fusion
        x = self.layer4(x)
        x = x + self.fusion4(dino_map, x.shape[-2:])
        features_list.append(x)  # 存入 Stage 4 特征

        # 后处理得到最终分类特征
        x_pool = self.global_avgpool(x)
        features = torch.flatten(x_pool, 1)
        logits = self.fc(features)

        if return_features:
            # 修改返回值：不仅返回 logits 和 features，还返回 features_list
            return logits, features, features_list
