...
ALL_METHODS = ['GDRNet', 'GDRNet_DUAL', 'GDRNet_Mask', 'GDRNet_MASK_SIAM', 'GDRNet_DUAL_MASK',  'GDRNet_DUAL_MASK_SIAM', 'ERM', 'GREEN', 'CABNet', 'MixupNet', 'MixStyleNet', 'Fishr', 'DRGen']
...
def init_log_test(args, cfg, log_path, train_loader_length, dataset_size):
    ...
    writer = SummaryWriter(os.path.join(log_path, 'tensorboard'))
    writer.add_text('config', str(args))
    # logging.basicConfig(filename=log_path + '/log_test_{}.txt'.format(cfg.DATASET.TARGET_DOMAINS), level=logging.INFO,format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.basicConfig(filename=log_path + '_test_{}.txt'.format(cfg.DATASET.TARGET_DOMAINS), level=logging.INFO,format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info("{} iterations per epoch".format(train_loader_length))
    logging.info("We have {} images in train set, {} images in val set, and {} images in test set.".format(dataset_size[0], dataset_size[1], dataset_size[2]))
    logging.info(str(args))
...
def get_scheduler(optimizer, max_epoch):
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[max_epoch * 1.0], gamma=0.1)
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[max_epoch * 0.5], gamma=0.1)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epoch/2, eta_min=0.00015)

    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epoch, eta_min=0.0001)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epoch, eta_min=0.00015)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epoch, eta_min=0.0)
    
    return scheduler

def get_scheduler2(optimizer, max_epoch):
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[max_epoch * 1.0], gamma=0.1)
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[max_epoch * 0.5], gamma=0.1)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epoch/2, eta_min=0.00015)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epoch, eta_min=0.0)
...
def get_scheduler_siam(optimizer, init_lr, epoch, max_epoch):
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[max_epoch * 1.0], gamma=0.1)
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[max_epoch * 0.5, max_epoch * 0.8], gamma=0.1)
    warmup_epoch=30
    if epoch <= warmup_epoch:
        cur_lr = init_lr * epoch / warmup_epoch 
    else:
        cur_lr = init_lr #* 0.5 #* (1. + math.cos(math.pi * (epoch - warmup_epoch - 1) / (max_epoch - warmup_epoch)))
    
    for param_group in optimizer.param_groups:
        if 'fix_lr' in param_group and param_group['fix_lr']:
            param_group['lr'] = init_lr #*5
        else:
            param_group['lr'] = cur_lr
...
def update_writer_siam_v2(writer, epoch, scheduler, loss_dict_iter):
    # logging.info('epoch: {}, total loss: {}'.format(epoch, loss_dict_iter.mean()))
    logging.info('epoch: {}, total loss: {}'.format(epoch, loss_dict_iter['loss']))
