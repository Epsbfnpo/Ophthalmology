...
def main():
    ...
    for i in iterator:
        ...
        # 添加 enumerate 获取 batch index (step)
        for step, (image, mask, label, domain, img_index) in enumerate(train_loader):
            ...
            # [Update] 传入 step 和 accum_iter 控制累积
            # 注意：仅 GDRNet 支持此参数，如果切换其他算法可能报错。
            # 为了稳健性，可以使用 kwargs 检查，但既然你明确只改 GDRNet，这里直接传
