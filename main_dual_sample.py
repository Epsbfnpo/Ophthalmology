    ...
    for i in iterator:
        ...
        for i, imgs in enumerate(train_loader):
            if cfg.TRANSFORM.FREQ: 
                image, image_freq, mask, label, domain, img_index=imgs
            else:
                # image, mask, label, domain, img_index=imgs
                imgs_a, imgs_b=imgs  ### a为随机采样，b为均匀采样
                image_a, mask_a, label_a, domain_a, img_index_a=imgs_a
                image_b, mask_b, label_b, domain_b, img_index_b=imgs_b
            ...
            if args.algorithm=='GDRNet_MASK_SIAM':
                get_scheduler_siam(algorithm.optimizer, cfg.LEARNING_RATE, epoch)

            algorithm.train()
            if cfg.TRANSFORM.FREQ: 
                minibatch = [image.cuda(), image_freq.cuda(), mask.cuda(), label.cuda().long(), domain.cuda().long()]
            else:
                minibatch = [[image_a.cuda(), mask_a.cuda(), label_a.cuda().long(), domain_a.cuda().long()],[image_b.cuda(), mask_b.cuda(), label_b.cuda().long(), domain_b.cuda().long()]]
